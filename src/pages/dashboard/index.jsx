...
export default function(props) {
  ...
  // 获取屏幕
  const {
    loading: getScreenLoading,
    data: screens,
    run: getScreen,
  } = useRequest(req.dashBoardScreenGet, {
    onSuccess: resp => {
      if (resp && resp.length) {
  ...
  // 修改或新增
  const { loading: editOrAddLoading, run: editOrAddReq } = useRequest(
    req.dashBoardScreenAddOrEdit,
    {
      manual: true,
      onSuccess: () => {
        getScreen();
  ...
  // 获取图表列表
  const { loading: getChatsLoading, run: getChats } = useRequest(
    req.dashBoardGet,
    {
      manual: true,
      onSuccess: resp => {
              ...
              screens && showScreen ? (
                <React.Fragment>
                  ...
                  <Link to={ROUTERS.dashBoardAdd + '?id=' + showScreen}>
                    <Button type={'primary'} title={'新增图表'}>
                      新增图表
                    </Button>
                  </Link>
                </React.Fragment>
